...
- trans: n×m transition function for deterministic problems or
         m×n×n transition probabilities for stochastic problem
- T: number of decision periods, if finite horizon
- vT: n terminal values, if finite horizon
"""
type DDModel
    β::Float64  # discount factor
    reward::Matrix{Float64}  # matrix of reward values
    trans::Union(Array{Float64,3},Matrix{Float64})  # n.m or m.n.n (3d=stochastic)
    T::Int  # number of decision periods, if finite horizon
    vT::Vector{Float64}  # n.1 terminal values, if finite horizon
end
